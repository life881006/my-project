...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.pageObj.everyPage = this.everyPage;//挂载到vue中的everyPage
			...
			this.$options.methods.getPageData.bind(this)(this.pageObj,this.transmitObj).then((data)=> {				
				this.pageObj.everyPage = data.everyPage;//挂载到vue中的everyPage
				this.pageObj.currentPage = data.currentPage;
				this.pageObj.totalCount = data.totalCount;
				this.tCount = data.totalCount;
				this.ePage = this.pageObj.everyPage;
				this.cPage = this.pageObj.currentPage;
				this.pageObj.path=this.$router.history.current.path;
				this.$store.dispatch("paginationAdd",this.pageObj);//将页码信息插入
				this.transmitObj.handler = "/https/news/getNewssByPage.do";
				return this.$options.methods.getPageData.bind(this)(this.pageObj,this.transmitObj)
			}).then((data)=>{
				this.tableData = data;
			});
		},
		methods: {
			handleCurrentChange(val) {
				...
				this.getPageData.bind(this)(this.pageObj,this.transmitObj).then((data)=> {
					this.tableData = data;
				})
			},
			setEveryPage(){
