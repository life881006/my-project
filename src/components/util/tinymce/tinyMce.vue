<template>
  <textarea
    ...
    :id="MceEl.name"
    :ref="MceEl.name"
    style="border:1px solid #dcdfe6;border-radius:4px;box-shadow: none;"
  ></textarea>
</template>
  ...
  methods: {
    tinyMceInit () {
      ...
      tinymce.init({
        ...
        style_formats:
          this.MceEl.styleFormats.length === 0
            ? styleFormats
            : this.MceEl.styleFormats,
    ...
    getMceContent () {
      let textContent = this.$trim(
        // eslint-disable-next-line no-undef
        tinymce.get(this.MceEl.name).getContent({ format: 'text' })
      )
