...
export default {
  ...
  methods: {
    ...
    isQuery (routeItem) {
      if (JSON.stringify(routeItem.query) === '{}') {
        // 验证是否带参数
    ...
    deleteCurrentTag () {
      ...
      let pathEnd =
        this.currentTagPath.indexOf('?') < 0
          ? this.currentTagPath.length
          : this.currentTagPath.indexOf('?')
...
.visitTags .routerTags:hover {
  cursor: pointer;
}
.visitTags .routerTags i {
  margin-left: 5px;
  color: #666;
  cursor: pointer;
  text-align: center;
